...
`gallery-examples.json` is generated by this repo, but its normalized record shape is defined as a [Table Schema](https://datapackage.org/standard/table-schema/) inside [`datapackage.json`](datapackage.json) under the `gallery_examples` resource. It has seven fields (`gallery_name`, `example_name`, `example_url`, `spec_url`, `categories`, `description`, `datasets`) with the stable `example_url` as its primary key and `spec_url` as an additional unique key. `fieldsMatch: "exact"` makes validation catch missing or unexpected output fields. Source galleries may publish richer indexes; the generator projects them into this common shape so downstream consumers (LLMs, search tools, and example aggregators) do not need per-source integrations.
